...
class WebViewImpl {
  constructor (webviewNode) {
    ...
    this.onVisibilityChanged(document.visibilityState)
  ...
  onVisibilityChanged (visibilityState) {
    // If the embedder window is hidden, the webview is hidden, but if the
    // window is visible, we should inherit the visibility of the <webview>
    // element.
    this.browserPluginNode.style.visibility = visibilityState === 'hidden' ? 'hidden' : 'inherit'

    // For some reason, we need to force a reflow to ensure that the webview
    // visibilitychange event fires.
    this.webviewNode.offsetLeft
  }
